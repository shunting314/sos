...
EthernetFrame::EthernetFrame(MACAddr dst_mac_addr, uint16_t etherType, uint8_t *data, int len, bool should_add_crc) : payload_len_(len) {
  frame_.dst_mac_addr_ = dst_mac_addr;
  frame_.src_mac_addr_ = self_mac;
  frame_.etherType_ = hton(etherType);
