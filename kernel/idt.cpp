...
extern "C" void interrupt_handler(int32_t intNum, InterruptFrame* framePtr) {
  ...
  if (intNum >= 32 && intNum < 48) { // irq
    ...
    if (irq_handlers[irq]) {
      ...
      // NOTE: Explicit EOI is needed for interrupts comming from the
      // wifi controller.
      //
      // Without this, we only get a single interrupt from the wifi
      // controller.
      //
      // NOTE: we already enable AEOI mode when remap the pic.
      //
      // This is not needed by the keyboard or timer interrupt. I guess
      // it's because those IRQ lines are not shared. While potentially
      // the IRQ for wifi controller may be shared with other devices.
      pic_send_eoi(irq);
    } else {
